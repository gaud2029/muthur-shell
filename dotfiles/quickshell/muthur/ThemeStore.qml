...
QtObject {
    ...
    // Keeps Hyprland's window borders in sync: dotfiles/hypr/hyprland.lua
    // require()s this file next to it (with a grey fallback when it's
    // missing) and Hyprland re-reads its config on `hyprctl reload`.
    readonly property FileView hyprlandTheme: FileView {
        path: Quickshell.env("HOME") + "/.config/hypr/colors.lua"
        printErrors: true
    }

    function writeHyprlandTheme() {
        const rgb = c => "rgb(" + c.toString().substring(1) + ")";
        root.hyprlandTheme.setText(
            "-- Generated by ThemeStore.qml; do not edit.\n" +
            "return {\n" +
            "\tfg = \"" + rgb(root.colorFg) + "\",\n" +
            "\tdim = \"" + rgb(root.colorDim) + "\",\n" +
            "}\n"
        );
        // Harmless no-op if Hyprland isn't the running compositor.
        Quickshell.execDetached(["hyprctl", "reload"]);
    }

    // Keeps niri's window borders in sync: dotfiles/niri/config.kdl
    // include()s this file next to it (a missing include is fatal to niri,
    // so install.sh seeds a grey one) and niri reloads the config on its own
    // when the file changes — no command to run.
    readonly property FileView niriTheme: FileView {
        path: Quickshell.env("HOME") + "/.config/niri/colors.kdl"
        printErrors: true
    }

    function writeNiriTheme() {
        root.niriTheme.setText(
            "// Generated by ThemeStore.qml; do not edit.\n" +
            "layout {\n" +
            "    border {\n" +
            "        active-color \"" + root.colorFg.toString() + "\"\n" +
            "        inactive-color \"" + root.colorDim.toString() + "\"\n" +
            "        urgent-color \"" + root.colorFocus.toString() + "\"\n" +
            "    }\n" +
            "}\n"
        );
    }
    ...
    // GTK colors (GitHub issue #5). GTK loads ~/.config/gtk-{3,4}.0/gtk.css
    // on top of its theme, so these override libadwaita (GTK4, through its
    // CSS variables) and adw-gtk3 (GTK3, through the same names as
    // @define-color) without shipping a theme of our own. GTK re-reads the
    // file when the theme changes, which the light/dark swap in
    // writeColorScheme() triggers; a switch between two dark (or two
    // light) presets reaches already-open apps on their next start.
    readonly property FileView gtk3Css: FileView {
        path: Quickshell.env("HOME") + "/.config/gtk-3.0/gtk.css"
        printErrors: true
    }
    readonly property FileView gtk4Css: FileView {
        path: Quickshell.env("HOME") + "/.config/gtk-4.0/gtk.css"
        printErrors: true
    }

    // "#rrggbb" for any color value, and `b` laid over `a` at opacity `t`.
    function hex(c) {
        return Qt.color(c).toString();
    }
    function mix(a, b, t) {
        return hex(Qt.tint(a, Qt.alpha(b, t)));
    }

    function writeGtkCss() {
        const hex = root.hex, mix = root.mix;
        const bg = hex(root.colorBg), fg = hex(root.colorFg);
        const dim = hex(root.colorDim), focus = hex(root.colorFocus);
        const raised = hex(root.color(0));
        const bar = mix(bg, fg, 0.08);
        const side = mix(bg, fg, 0.04);
        const selected = mix(bg, focus, 0.30);
        // libadwaita's color names; GTK4 takes each as a --variable too.
        const colors = [
            ["window_bg_color", bg], ["window_fg_color", fg],
            ["view_bg_color", raised], ["view_fg_color", fg],
            ["headerbar_bg_color", bar], ["headerbar_fg_color", fg],
            ["headerbar_border_color", dim], ["headerbar_backdrop_color", bg],
            ["sidebar_bg_color", side], ["sidebar_fg_color", fg],
            ["sidebar_backdrop_color", bg], ["sidebar_border_color", dim],
            ["secondary_sidebar_bg_color", side], ["secondary_sidebar_fg_color", fg],
            ["secondary_sidebar_backdrop_color", bg], ["secondary_sidebar_border_color", dim],
            ["card_bg_color", raised], ["card_fg_color", fg],
            ["popover_bg_color", raised], ["popover_fg_color", fg],
            ["dialog_bg_color", bg], ["dialog_fg_color", fg],
            ["thumbnail_bg_color", raised], ["thumbnail_fg_color", fg],
            ["accent_bg_color", focus], ["accent_fg_color", bg], ["accent_color", focus],
            ["destructive_bg_color", root.color(1)], ["destructive_fg_color", bg],
            ["destructive_color", root.color(1)],
            ["success_bg_color", root.color(2)], ["success_fg_color", bg], ["success_color", root.color(2)],
            ["warning_bg_color", root.color(3)], ["warning_fg_color", bg], ["warning_color", root.color(3)],
            ["error_bg_color", root.color(1)], ["error_fg_color", bg], ["error_color", root.color(1)],
            ["border_color", dim], ["borders", dim]
        ];
        const header = "/* Generated by ThemeStore.qml (writeGtkCss) — do not edit by\n" +
            " * hand, changes are overwritten on every preset switch. */\n\n";
        const defines = colors.map(c => "@define-color " + c[0] + " " + c[1] + ";").join("\n") + "\n";
        // Selected rows, in lists, grids, sidebars and GTK3 tree views.
        const selection = sel => "\n" + sel + " {\n  background-color: " + selected +
            ";\n  color: " + fg + ";\n}\n";
        root.gtk3Css.setText(header + defines +
            "@define-color theme_selected_bg_color " + selected + ";\n" +
            "@define-color theme_selected_fg_color " + fg + ";\n" +
            selection("row:selected, treeview.view:selected, iconview:selected, " +
                "flowbox > flowboxchild:selected"));
        root.gtk4Css.setText(header + defines +
            "\n:root {\n" +
            colors.filter(c => c[0] !== "borders")
                .map(c => "  --" + c[0].replace(/_/g, "-") + ": " + c[1] + ";").join("\n") +
            "\n}\n" +
            selection("row:selected, gridview > child:selected, " +
                "columnview > listview > row:selected, flowbox > flowboxchild:selected"));
    }

    // Firefox's own chrome (GitHub issue #6): tab strip, toolbars, URL
    // bar, menus and panels, through the CSS variables its default theme
    // is built from. scripts/firefox-theme.py drops the file into each
    // profile Firefox starts as chrome/muthur.css, @imports it from
    // userChrome.css and enables userChrome.css in user.js. Pages follow
    // light/dark on their own (writeColorScheme); the chrome needs a
    // Firefox restart, since it only reads userChrome.css at startup.
    function writeFirefoxCss() {
        const hex = root.hex, mix = root.mix;
        const bg = hex(root.colorBg), fg = hex(root.colorFg);
        const dim = hex(root.colorDim), focus = hex(root.colorFocus);
        const raised = hex(root.color(0));
        const bar = mix(bg, fg, 0.08);
        const selected = mix(bg, focus, 0.30);
        // Names as of Firefox 155 (toolkit design-system tokens).
        const vars = [
            // Tab strip, and the selected tab joined to the toolbar below.
            ["--toolbox-background-color", bg], ["--toolbox-text-color", fg],
            ["--toolbox-background-color-inactive", bg], ["--toolbox-text-color-inactive", dim],
            ["--lwt-accent-color", bg], ["--lwt-text-color", fg],
            ["--tab-background-color-selected", bar], ["--tab-selected-textcolor", fg],
            ["--tab-background-color-hover", mix(bg, fg, 0.05)],
            ["--tab-selected-outline-color", dim],
            // Navigation toolbar and bookmarks bar.
            ["--toolbar-background-color", bar], ["--toolbar-text-color", fg],
            ["--toolbar-color", fg], ["--toolbarbutton-icon-fill", fg],
            ["--chrome-content-separator-color", dim],
            // URL and search fields, and their dropdown.
            ["--toolbar-field-background-color", raised], ["--toolbar-field-text-color", fg],
            ["--toolbar-field-border-color", dim],
            ["--toolbar-field-background-color-focus", raised], ["--toolbar-field-text-color-focus", fg],
            ["--toolbar-field-border-color-focus", focus],
            ["--urlbarview-background-color-selected", selected], ["--urlbarview-text-color-selected", fg],
            ["--urlbarview-background-color-hover", mix(raised, fg, 0.06)],
            ["--urlbarview-separator-color", dim],
            // Sidebar.
            ["--sidebar-background-color", bg], ["--sidebar-text-color", fg],
            ["--sidebar-border-color", dim],
            // Accent: focus rings, checkboxes, primary buttons.
            ["--focus-outline-color", focus], ["--color-accent-primary", focus],
            ["--color-accent-primary-hover", mix(focus, fg, 0.15)],
            ["--color-accent-primary-active", mix(focus, bg, 0.15)],
            ["--button-text-color-primary", bg],
            // Behind a page while it loads.
            ["--tabpanel-background-color", bg]
        ];
        // Menus and popups set their colors on the popup element itself.
        const panelVars = [
            ["--panel-background-color", raised], ["--panel-text-color", fg],
            ["--panel-border-color", dim], ["--panel-separator-color", dim]
        ];
        const block = (sel, list) => sel + " {\n" +
            list.map(v => "  " + v[0] + ": " + v[1] + " !important;").join("\n") + "\n}\n";
        // !important: Firefox's default theme sets several of these on the
        // root element itself, which a plain :root rule would lose to.
        const css = "/* Generated by ThemeStore.qml (writeFirefoxCss) — do not edit by\n" +
            " * hand, changes are overwritten on every preset switch. */\n\n" +
            block(":root", vars) + "\n" + block(":root, menupopup, panel", panelVars);
        Quickshell.execDetached([Quickshell.shellDir + "/scripts/firefox-theme.py", css]);
    }
